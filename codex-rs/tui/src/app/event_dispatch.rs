    ...
    pub(super) async fn handle_event(
        ...
        match event {
            ...
            AppEvent::MultiAiCodeImSubmitUserMessage {
                ...
                let declined_approvals = if remote_im_input {
                    self.decline_pending_exec_approvals_before_remote_im_input(app_server)
                        .await
                        .map_err(|err| format!("failed to release pending approval: {err:#}"))
                } else {
                    Ok(Vec::new())
                };
                match declined_approvals {
                    Err(message) => crate::multi_ai_code_im_bridge::send_control_result(
                        &request_id,
                        false,
                        "",
                    ...
                    Ok(declined_approvals) => {
                        match self.chat_widget.submit_user_message_from_remote_im(
                            ...
                            Ok(()) => {
                                let declined_approval_metadata = declined_approvals
                                    .iter()
                                    .map(|approval| {
                                        (
                                            approval.approval_id.as_str(),
                                            approval.command_summary.as_str(),
                                        )
                                    })
                                    .collect::<Vec<_>>();
                                crate::multi_ai_code_im_bridge::send_user_message_control_result(
                                    &request_id,
                                    true,
                                    "queued",
                                    None,
                                    &declined_approval_metadata,
                                );
                            }
                            Err(message) => {
                                crate::multi_ai_code_im_bridge::send_control_result(
            ...
            AppEvent::MultiAiCodeImResolveApproval {
                ...
                let result = match parsed_thread_id {
                    Ok(thread_id) => {
                        if let Some(winner) = self
                            .pending_app_server_requests
                            .resolved_exec_approval(&thread_id.to_string(), &approval_id)
                            .cloned()
                        {
                            tracing::info!(
                                %thread_id,
                                ...
                                loser = crate::app::app_server_requests::ExecApprovalResolutionSource::RemoteDecision.as_str(),
                                "ignored a late Remote IM approval decision"
                            );
                            Ok((false, winner.decision, winner.source))
                        } else {
                            match self
                                .chat_widget
                                .remote_im_exec_approval_decision(&approval_id, &decision)
                            {
                                ...
                                Ok(decision) => {
                                    if let Err(message) = self
                                        .chat_widget
                                        .validate_remote_im_exec_approval(
                                            thread_id,
                                            &turn_id,
                                            &task_id,
                                            &approval_id,
                                    ...
                                    {
                                        Err(message)
                                    } else if !self.pending_app_server_requests.has_exec_approval(
                                        &thread_id.to_string(),
                                        &approval_id,
                                    ) {
                                        Err("approval is no longer pending".to_string())
                                    } else {
                                        let applied_decision = decision.clone();
                                        ...
                                        match self
                                            .try_resolve_app_server_request(
                                                ...
                                                thread_id,
                                                &op,
                                                crate::app::app_server_requests::ExecApprovalResolutionSource::RemoteDecision,
                                            )
                                            .await
                                        {
                                            Ok(true) => {
                                                ...
                                                self.chat_widget
                                                    .dismiss_app_server_request(&resolved);
                                                Ok((
                                                    true,
                                                    applied_decision,
                                                    crate::app::app_server_requests::ExecApprovalResolutionSource::RemoteDecision,
                                            ...
                                            Err(err) => Err(format!(
                                                "failed to resolve approval: {err}"
                                            )),
                                        }
                                    }
                                }
                ...
                match result {
                    Ok((applied, winner_decision, winner_source)) => {
                        crate::multi_ai_code_im_bridge::send_approval_control_result(
                        &request_id,
                            applied,
                            &winner_decision,

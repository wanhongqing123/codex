...
async fn windows_sandbox_prompt_withholds_admin_setup_when_helper_is_not_bundled() {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    let _guard =
        crate::windows_sandbox::test_support::set_elevated_setup_available(/*value*/ false);
...
async fn choosing_to_continue_without_a_sandbox_closes_the_prompt_and_changes_nothing() {
    ...
    chat.set_feature_enabled(Feature::WindowsSandboxElevated, /*enabled*/ false);
    let approval_before = chat.config.permissions.approval_policy.clone();
    let mode_before = chat.windows_sandbox_config.mode;
    ...
    assert_eq!(
        chat.windows_sandbox_config.mode, mode_before,
...
async fn required_elevated_sandbox_reports_missing_helper_without_downgrading() {
    ...
    chat.set_feature_enabled(Feature::WindowsSandboxElevated, /*enabled*/ false);
    chat.windows_sandbox_config.requirements = Some(Some(vec![WindowsSandboxSetupMode::Elevated]));
    chat.set_windows_sandbox_mode(Some(WindowsSandboxSetupMode::Elevated));
...
async fn suppressed_host_does_not_offer_the_optional_windows_sandbox() {
    ...
    let _available =
        crate::windows_sandbox::test_support::set_elevated_setup_available(/*value*/ false);
    let _suppressed = crate::windows_sandbox::test_support::set_optional_prompt_suppressed(true);
...
async fn the_same_session_is_still_offered_a_sandbox_when_not_suppressed() {
    ...
    assert!(
        chat.has_active_view(),
        "the unsuppressed session must still ask"
    );
}
...
async fn suppression_also_withdraws_the_onboarding_promise_to_create_a_sandbox() {
    ...
    assert!(
        crate::windows_sandbox::trust_screen_may_promise_sandbox(&chat.windows_sandbox_config),
...
async fn a_configured_sandbox_mode_is_unaffected_by_suppression() {
    ...
    assert_eq!(
        chat.windows_sandbox_config.level(),
...
#[tokio::test]
async fn missing_windows_helper_does_not_weaken_server_required_sandbox() {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    let _helper =
        crate::windows_sandbox::test_support::set_elevated_setup_available(/*value*/ false);
    chat.windows_sandbox_config.mode = Some(WindowsSandboxSetupMode::Elevated);
    chat.windows_sandbox_config.requirements = Some(Some(vec![WindowsSandboxSetupMode::Elevated]));
    let before = chat.windows_sandbox_config.clone();
    let preset = builtin_approval_presets()
        .into_iter()
        .find(|preset| preset.id == "auto")
        .expect("auto preset");
    chat.open_windows_sandbox_enable_prompt(preset, /*profile_selection*/ None);
    let popup = render_bottom_popup(&chat, /*width*/ 120);
    assert!(popup.contains("does not include"), "{popup}");
    assert!(!popup.contains("Continue without a sandbox"), "{popup}");
    assert!(!popup.contains("Use non-admin sandbox"), "{popup}");
    assert_eq!(chat.windows_sandbox_config, before);
}

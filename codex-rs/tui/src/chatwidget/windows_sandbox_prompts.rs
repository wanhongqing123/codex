...
impl ChatWidget {
    ...
    pub(crate) fn windows_sandbox_may_stay_disabled(&self) -> bool {
        matches!(self.windows_sandbox_config.requirements, Some(None))
    }
    ...
    pub(crate) fn open_windows_sandbox_enable_prompt(
        ...
        if !self.windows_sandbox_config.is_enabled() && self.windows_sandbox_may_stay_disabled() {
            items.push(SelectionItem {
    ...
    pub(crate) fn maybe_prompt_windows_sandbox_enable(&mut self, show_now: bool) {
        ...
        let offer_optional_setup = !self.windows_sandbox_config.is_enabled()
            && can_offer_any_mode

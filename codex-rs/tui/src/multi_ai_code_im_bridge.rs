...
pub(crate) fn send_approval_resolved(
    ...
    reply_id: Option<&str>,
    task_id: Option<&str>,
    decision: Option<&codex_app_server_protocol::CommandExecutionApprovalDecision>,
...
fn approval_resolved_payload(
    ...
    if let Some(decision) = decision {
        payload["approvalDecision"] = approval_decision_name(decision).into();
    }
    if let Some(resolution_source) = resolution_source {
        payload["resolutionSource"] = resolution_source.into();
    }
    payload
}

fn approval_decision_name(
    decision: &codex_app_server_protocol::CommandExecutionApprovalDecision,
) -> &'static str {
    use codex_app_server_protocol::CommandExecutionApprovalDecision;
    match decision {
        CommandExecutionApprovalDecision::Accept => "accept",
        CommandExecutionApprovalDecision::AcceptForSession => "accept-for-session",
        ...
        CommandExecutionApprovalDecision::Cancel => "cancel",
    }
}
...
pub(crate) fn send_control_result(request_id: &str, ok: bool, text: &str, error: Option<&str>) {
    send_control_result_with_metadata(request_id, ok, text, error, &[], None);
}

pub(crate) fn send_user_message_control_result(
    request_id: &str,
    ok: bool,
    text: &str,
    error: Option<&str>,
    auto_declined_approvals: &[(&str, &str)],
) {
    send_control_result_with_metadata(request_id, ok, text, error, auto_declined_approvals, None);
}

pub(crate) fn send_approval_control_result(
    request_id: &str,
    applied: bool,
    winner_decision: &codex_app_server_protocol::CommandExecutionApprovalDecision,
    winner_source: &str,
) {
    send_control_result_with_metadata(
        request_id,
        true,
        if applied {
            "approval resolved"
        ...
        None,
        &[],
        Some((applied, winner_decision, winner_source)),
    );
}

fn send_control_result_with_metadata(
    request_id: &str,
    ok: bool,
    text: &str,
    error: Option<&str>,
    auto_declined_approvals: &[(&str, &str)],
    approval_resolution: Option<(
        bool,
    ...
    if !auto_declined_approvals.is_empty() {
        payload["autoDeclinedApprovals"] = json!(
            auto_declined_approvals
                .iter()
                .map(|(approval_id, command_summary)| json!({
                    "approvalId": approval_id,
    ...
    if let Some((applied, winner_decision, winner_source)) = approval_resolution {
        payload["approvalResolution"] = json!({
            "applied": applied,
            "winnerDecision": approval_decision_name(winner_decision),
...
mod tests {
    ...
    #[test]
    fn approval_resolution_payload_carries_the_winning_decision_and_source() {
        let thread_id = ThreadId::new();
        let payload = approval_resolved_payload(
            thread_id,
            "turn-1",
            ...
            Some("reply-1"),
            Some("task-1"),
            Some(&codex_app_server_protocol::CommandExecutionApprovalDecision::Decline),
            Some("remote-im-input"),
        );

        assert_eq!(payload["kind"], "approval_resolved");
        assert_eq!(payload["approvalId"], "approval-1");
        assert_eq!(payload["threadId"], thread_id.to_string());
        assert_eq!(payload["turnId"], "turn-1");
        assert_eq!(payload["replyId"], "reply-1");
        assert_eq!(payload["taskId"], "task-1");
        assert_eq!(payload["approvalDecision"], "decline");
        assert_eq!(payload["resolutionSource"], "remote-im-input");
    }

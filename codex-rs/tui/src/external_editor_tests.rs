...
#[test]
fn editor_directory_creates_missing_configured_home() {
    let paths = EditorPaths::new();
    fs::remove_dir(&paths.codex_home).expect("remove empty Codex home");
    let policy = workspace_write_policy(&[]);

    let directory = editor_directory(&paths.codex_home, &policy, &paths.cwd)
        .expect("create configured Codex home and editor directory");
...
#[tokio::test]
async fn editor_process_reports_configured_home_creation_error() {
    let paths = EditorPaths::new();
    fs::remove_dir(&paths.codex_home).expect("remove empty Codex home");
    fs::write(&paths.codex_home, "occupied").expect("occupy configured home with a file");
    let policy = workspace_write_policy(&[]);
    let command = vec!["unused-editor".to_string()];

    let error = run_editor("seed", &command, &paths.codex_home, &policy, &paths.cwd)
        .await
        .expect_err("report the configured home error before launching an editor");

    let io_error = error
        .downcast_ref::<std::io::Error>()
        .expect("filesystem error");
    assert_eq!(io_error.kind(), std::io::ErrorKind::NotADirectory);
    assert_eq!(fs::read_to_string(&paths.codex_home).unwrap(), "occupied");
}

#[tokio::test]
async fn editor_process_enforces_configured_home_buffer_policy() {
    let paths = EditorPaths::new();
    let policy = workspace_write_policy(&[&paths.codex_home]);
    let command = vec!["unused-editor".to_string()];
    ...
    let error = run_editor("seed", &command, &paths.codex_home, &policy, &paths.cwd)
        .await
        .expect_err("reject a sandbox-writable buffer directory before launching an editor");

    assert_eq!(error.to_string(), "editor directory must not be writable");
